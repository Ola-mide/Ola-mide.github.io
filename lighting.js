...
// Fetching the data from sheets from each column
...
fetch(`${url}?header=Growlight1`)
    ...
    .then(({ data }) => {
        console.log(data);
        if (data == "ON") {
            document.getElementById("Growlight1").checked = true;
        } else if (data == "OFF") {
            document.getElementById("Growlight1").checked = false;
        }
    })
    .catch((error) => console.error("!!!!!!!!", error));
...
fetch(`${url}?header=Growlight2`)
    ...
    .then(({ data }) => {
        console.log(data);
        if (data == "ON") {
            document.getElementById("Growlight2").checked = true;
        } else if (data == "OFF") {
            document.getElementById("Growlight2").checked = false;
        }
    })
    .catch((error) => console.error("!!!!!!!!", error));
...
const updateLight1 = () => {
    fetch(`${url}?header=Growlight1`)
        ...
        .then(({ data }) => {
            ...
            console.info("Updating light1 status..");
        })
        .catch((error) => console.error("!!!!!!!!", error));
};
...
const updateLight2 = () => {
    fetch(`${url}?header=Growlight2`)
        ...
        .then(({ data }) => {
            ...
            console.info("Updating light2 status..");
        })
        .catch((error) => console.error("!!!!!!!!", error));
};
// starting fetch
...
autoRefresh({
    anyFunction: updateLight1,
    interval: 3500,
});
autoRefresh({
    anyFunction: updateLight2,
    interval: 3500,
});
autoRefresh({
...
// Checking light status data
var lightstat1 = "";
var lightstat2 = "";
function handleClick(cb) {
    if (document.getElementById("Growlight1").checked == true) {
        lightstat1 = "ON";
    } else {
        lightstat1 = "OFF";
    }

    if (document.getElementById("Growlight2").checked == true) {
        lightstat2 = "ON";
    } else {
        lightstat2 = "OFF";
    }
    ...
    keyValuePairs.push("Growlight1" + "=" + lightstat1);
    keyValuePairs.push("Growlight2" + "=" + lightstat2);
    // not on page
